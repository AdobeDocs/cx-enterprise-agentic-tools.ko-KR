...
모든 무의미한 도구들이 같은 요구를 제공하는 것은 아니다. 각 사용자가 수행하는 작업, 사용 시기 및 시작 방법을 살펴봄으로써 상황에 맞는 시작점을 선택할 수 있습니다.
...
에이전트 기술은 에이전트가 따를 수 있는 지침으로 인코딩된 Adobe의 도메인 전문 기술입니다. CX 엔터프라이즈 워크플로우에 맞게 조정되고 안정적이며 반복적으로 수행할 수 있는 작업을 에이전트가 정확하게 판단해 주기를 바라는 대신
...
이러한 도구는 함께 작동하도록 설계되었으며 이를 결합하여 Adobe AI을 최대한 활용할 수 있습니다. 에이전트 기술은 AI 클라이언트가 MCP 서버를 사용하여 CX 엔터프라이즈 워크플로우에 대한 올바른 경로를 에이전트를 유지하는 방법을 안내할 수 있습니다. 또한 스킬은 API를 호출하는 방법과 시기를 알려 주며, 사용자 지정 제작 자동화에 Adobe 모범 사례 가드레일을 추가할 수 있습니다. 하나만 고르실 필요는 없습니다
