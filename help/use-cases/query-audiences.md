...
description: CX Coworker Gateway를 사용하여 대상자 활성화 상태를 모니터링하고, 대상 상태를 확인하고, 캠페인에 영향을 미치기 전에 문제를 노출할 수 있습니다.
...
캠페인이 시작되기 전이나 성과가 낮은 경우, 어떤 대상이 라이브인지, 어디에서 오는지, 대상이 정상인지 파악하는 것은 매우 중요합니다. 이 연습에서는 CX Coworker Gateway를 사용하여 Real-Time CDP을 열지 않고도 몇 초 만에 대상 상태 및 대상 상태를 파악할 수 있도록 AI 클라이언트를 통해 전체 활성화 그림을 얻는 방법을 보여 줍니다.
...
CX Coworker Gateway를 사용자 정의 커넥터로 연결하여 Real-Time CDP 도구에 액세스합니다.
...
CX Coworker Gateway의 RTCDP 툴은 읽기 전용이며 활성화 상태, 대상 상태 및 데이터 흐름 데이터를 표시하지만 구성을 수정하지 않습니다. 문제를 식별하면 애플리케이션에서 수정 사항이 발생합니다.
...
>[!NOTE]
>
>CX Coworker Gateway의 RTCDP 도구에는 대상 및 활성화 데이터가 표시되지만 대상 구성, 세그먼트 정의 또는 데이터 흐름 설정을 수정할 수 없습니다. 수정 단계는 Real-Time CDP 애플리케이션에서 발생합니다.
...
CX Coworker Gateway의 Real-Time CDP 툴은 다양한 대상 및 활성화 쿼리를 지원합니다. 동일한 세션에서 시도할 수 있는 프롬프트를 보려면 아래 시나리오를 확장하십시오.
